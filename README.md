...
Tank Trouble 스타일의 실시간 온라인 멀티플레이어 탱크 게임입니다. 초대 코드로 방을 만들고 친구를 초대해서 최대 8명까지 자유대전으로 함께 플레이할 수 있습니다.

## 주요 기능

- 닉네임 입력 후 "방 만들기" 또는 "초대 코드"로 참가
- 방에 모인 인원(2~8명)이 모두 "준비완료"를 누르면 자동으로 게임 시작
- 호스트가 맵 선택 가능 (미로 / 평지 / 벙커 아레나), 탱크 속도·연사 속도·총알 속도도 방장이 자유롭게 조절 가능
- 각 플레이어는 개인 화면에서 시야 범위(FOV, 줌 인/아웃)를 취향대로 조절 가능
- 탱크 이동(W A S D) + 발사(Space, 모바일은 화면 조이스틱 + 발사 버튼) — 키 입력에 즉시 반응하는 조작감, 총알이 벽에 맞으면 반사되어 튕겨나감
- 자신이 쏜 총알에는 맞지 않음
